...
static nsSinceEpoch msMonotonicTimeStart = 0;
...
Hal_getMonotonicTimeInMs()
{
  ...
  nsSinceEpoch msTime = now.tv_sec * 1000UL;
  msTime += now.tv_nsec / 1000000UL;

  if(msMonotonicTimeStart == 0) {
    msMonotonicTimeStart = msTime;
  }
...
nsSinceEpoch
Hal_getMonotonicStartTimeInMs()
{
    return msMonotonicTimeStart;
}
