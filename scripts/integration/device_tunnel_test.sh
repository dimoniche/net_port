...
prepare_test_device() {
  ...
  psql_exec "
    INSERT INTO devices (
      ...
      gen_random_uuid(), '${DEVICE_ID}', 'Integration Test Device', '${status}', '${token_hash}',
...
cleanup_test_device() {
  ...
  psql_exec "
    UPDATE devices
    SET status = 'inactive', assigned_port = NULL, updated_at = NOW()
...
main() {
  local response status message session_token assigned_port tunnel_port
  ...
  if [ "$status" = "authenticated" ]; then
    ...
    session_token="$(json_field session_token "$response")"
